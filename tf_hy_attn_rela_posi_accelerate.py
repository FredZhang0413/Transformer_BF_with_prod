...
def sum_rate(H, W, sigma2=1.0):
    ...
    SINR = signal_power / (interference + sigma2)  # (B, num_users)
...
class BeamformingTransformer(nn.Module):
    def __init__(self, config):
        ...
        # # Absolute Positional embeddings. (divided: H, W, P)
    ...
    def forward(self, H, W_prev):
        ...
        # # Add absolute positional embedding to Value tokens (antenna-level).
...
def train_beamforming_transformer(config, pretrained_path: str = None, history_path: str = None):
    ...
    grad_norm_history = [] ### record the variation of gradient norm

    for epoch in range(start_epoch_prev, config.max_epoch):

        ## relative weight: unsupervised loss / supervised loss
        if epoch < 100:
            student_weight = 0.5
            ...
            progress = epoch - 200
            decay_span = 500
            student_weight = 1.0 + 0.5 * (1 - math.cos(math.pi * progress / decay_span))
        else:
            student_weight = 2.0

        # # # Soft switch learning policy.
        # student_weight = max(0.0, 1 - (epoch / 100))
        # # student_weight = 0.0  
        ...
        max_rate = 0
        epoch_grad_norm = 0
        # max_norm_th = 3.0 + 7.0 * (epoch / config.max_epoch)
        for batch in pbar:
            ...
            W0 = compute_mmse_beamformer(H_mat, config.num_users, config.num_tx, 
            ...
            loss_supervised = config.mse_loss_scale * (total_mse_loss / config.T)
            loss_unsupervised = - total_rate / config.T
            # rl_scale = - student_weight * (loss_supervised.item() / loss_unsupervised.item())
            unsup_sup_ratio = loss_supervised / loss_unsupervised
            rl_scale = - student_weight * unsup_sup_ratio.item()
            loss_unsupervised = rl_scale * loss_unsupervised
            loss = loss_unsupervised + loss_supervised ## supervised MSE
            # print(loss_unsupervised.item(), loss_supervised.item())
            # bp()
            ...
            # print(f"Current gradient norm: {current_grad_norm:.4f}")
            # bp()

            # if current_grad_norm > 5 * max_norm_th:
            #     max_norm_th = 5
            # elif current_grad_norm > max_norm_th and current_grad_norm < 5 * max_norm_th:
            #     max_norm_th = min(1.5 * max_norm_th, 20)
            # elif current_grad_norm < max_norm_th:
            #     max_norm_th = max(0.75 * max_norm_th, 5)

            th.nn.utils.clip_grad_norm_(model.parameters(), max_norm=100)
            ...
            epoch_rate += ave_rate
            epoch_sr_loss -= loss_unsupervised.item()
            epoch_mse_loss += loss_supervised.item()
        ...
        print(f"Epoch {epoch+1} Average Sum Rate: {ave_pbar_rate:.4f}, Learning Rate: {current_lr:.2e}, student_weight: {student_weight:.2f}, MSE Loss: {ave_mse_loss:.4f}, SR Loss: {ave_sr_loss:.4f}, Gradient Norm: {ave_grad_norm:.4f}")
...
class BeamformerTransformerConfig:
    def __init__(self, **kwargs):
        ...
        self.mse_loss_scale = kwargs['mse_loss_scale']
