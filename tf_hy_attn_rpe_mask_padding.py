...
class BeamformingTransformer(nn.Module):
    def __init__(self, config):
        ...
        # self.out_proj_base = nn.Sequential(
        ...
        self.out_proj_dynamic = DynamicOutputProjection(total_tokens * config.d_model, config.d_model)
    ...
    ### expect invariant input size of H, W, HW
    def forward(self, H, W_prev, P_mask, user_attn_mask=None, antenna_attn_mask=None, input_mask = None, activated_degree=None):
        """
        Args:
        ...
        if activated_degree is None:
            activated_degree = config.num_users 

        W_next_vec = self.out_proj_dynamic(x_fused, activated_degree)  # (B, 2*activated_degree*activated_degree)
        W_real_active = W_next_vec[:, :activated_degree * activated_degree].reshape(-1, activated_degree, activated_degree)
        W_imag_active = W_next_vec[:, activated_degree * activated_degree:].reshape(-1, activated_degree, activated_degree)
        W_complex_active = W_real_active + 1j * W_imag_active # (B, m, m)
        W_norm_active = th.sqrt((W_complex_active.conj() * W_complex_active).sum(dim=(-2, -1)).real)
        W_next_active = W_complex_active / W_norm_active.view(-1, 1, 1) # (B, m, m)

        W_real = th.zeros(B, config.num_users, config.num_tx, device=device)
        W_imag = th.zeros(B, config.num_users, config.num_tx, device=device)

        nonzero_indices = th.nonzero(input_mask[0], as_tuple=True)
        i_idx = nonzero_indices[0][:activated_degree] # activated user idx
        j_idx = nonzero_indices[1][:activated_degree] # activated antenna idx

        batch_indices = th.arange(B, device=device).view(-1, 1, 1) # (B, 1, 1)
        batch_indices = batch_indices.expand(-1, activated_degree, activated_degree) # (B, m, m)
        user_indices = i_idx.view(1, -1, 1).expand(B, -1, activated_degree) # (B, m, m)
        ant_indices = j_idx.view(1, 1, -1).expand(B, activated_degree, -1) # (B, m, m)
        
        W_real[batch_indices, user_indices, ant_indices] = W_real_active
        W_imag[batch_indices, user_indices, ant_indices] = W_imag_active
        W_complex = W_real + 1j * W_imag # (B, K, N)

        # # Project fused features to final beamformer vector and normalize.
        # W_next_vec = self.out_proj_base(x_fused)  # (B, beam_dim)
        ...
        W_next = W_complex / W_norm.view(-1, 1, 1)
        # norm = th.norm(W_next, dim=1, keepdim=True)
        # W_next = W_next / norm
        return W_next, W_next_active
...
def train_beamforming_transformer(config, pretrained_path: str = None, history_path: str = None):
    ...
    for epoch in range(start_epoch_prev, config.max_epoch):
        ...
        for batch in pbar:
            ...
            W_next, W_next_active = model(H_mask, W_mask, P_mask, user_attn_mask=user_attn_mask, antenna_attn_mask=antenna_attn_mask, input_mask=M_out, activated_degree=activated_degree)
            W_next = W_next.transpose(-1, -2)  # (B, num_tx, num_users)

            # W_next_out = W_next * M_out
            # W_next_mask = W_next_out[:, i_idx][:, :, j_idx] # (B, m, m)
            # W_next_mask = W_next_mask.transpose(-1, -2)  # (B, m, m)
    
            # W_next_active = W_next_active.transpose(-1, -2)  # (B, m, m)
            ...
            # total_rate_act = sum_rate(H_act, W_next_active, sigma2=config.sigma2)
            # total_rate_mask = sum_rate(H_act, W_next_mask, sigma2=config.sigma2)
            total_rate = sum_rate(H_mask, W_next, sigma2=config.sigma2)
            # print(f"Full matrix Rate: {total_rate:.4f}, Mask matrix Rate: {total_rate_mask:.4f}, Active matrix Rate: {total_rate_act:.4f}")
