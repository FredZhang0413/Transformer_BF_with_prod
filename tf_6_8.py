...
def train_beamforming_transformer(config, pretrained_path: str = None, history_path: str = None):
    ...
    wmmse_prop = 0.25
    random_batch_size = int(math.ceil(config.batch_size * (1 - wmmse_prop)))  # random generated samples per batch (sum-rate objective in loss function)
    ...
    for epoch in range(start_epoch_prev, config.max_epoch):
        ...
        # teacher_weight = 0.0  
        wmmse_weight = 1.0 - teacher_weight  # Inverse of teacher weight
        ...
        for batch in pbar:
            ...
            for t in range(1, config.T + 1):
                ...
                # mse_loss_1 = fun.mse_loss(W_random_out, normlized_W0_random) ### a part of training batch 
                mse_loss_1 = fun.mse_loss(W_next, normlized_W0) ### the whole training batch
            ...
            loss_supervised_wmmse = (wmmse_weight * 20000) * (total_mse_loss_2 / config.T)
            ...
            pbar_batches += 1
            pbar.set_description(f"Epoch {epoch+1}, Avg Rate: {ave_rate:.4f}, Max Rate: {max_rate:.4f}, mmse MSE: {loss_supervised_mmse.item():.4f}, wmmse MSE: {loss_supervised_wmmse.item():.4f}")
