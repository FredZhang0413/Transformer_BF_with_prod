...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, query, key, value):
        ...
        out_mlp = out_mlp + self.mlp(self.ln2(out_mlp))  # Apply LayerNorm and MLP
        return out_mlp
...
def train_beamforming_transformer(config):
    ...
    scheduler = th.optim.lr_scheduler.LambdaLR(
        optimizer, lambda epoch: 1 - (config.lr_decay) * (epoch / config.max_epoch)
    )
    
    global mmse_rate_printed
    mmse_rate_printed = False
    model.train()
    teacher_weight = 0.0  # 0 or 1 depending on epoch switching
    rate_history = []
    test_rate_history = []
    ave_rate_history = []
    ...
    for epoch in range(config.max_epoch):
        ...
        # # Multi-stage Hard-switch learning policy.
        # if epoch < 50:
        #     teacher_weight = 1
        # elif epoch < 100 and epoch >= 50:
        #     teacher_weight = 0.8
        # elif epoch < 150 and epoch >= 100:
        #     teacher_weight = 0.6
        # elif epoch < 200 and epoch >= 150:
        #     teacher_weight = 0.4
        # elif epoch < 250 and epoch >= 200:
        #     teacher_weight = 0.2
        # else:
        #     teacher_weight = 0.0

        # Soft switch learning policy.
        teacher_weight = max(0.1, 1.0 - (epoch / config.cl_epoch))
        ...
        for batch in pbar:
            ...
            # print(f"Epoch {epoch+1}, Batch {pbar_batches+1}, Loss: {loss_unsupervised:.4f}, Supervised Loss: {10000*loss_supervised:.4f}")
            # loss = (1 - teacher_weight) * loss_unsupervised + (teacher_weight * 10000) * loss_supervised ## supervised MSE, 32*32
            loss = (1 - teacher_weight) * loss_unsupervised + (teacher_weight * 2000) * loss_supervised ## supervised MSE, 16*16
    ...
    plt.title(f"Training and Testing Rate when N={config.num_tx} and K={config.num_users} without CL")
    plt.legend()
    plt.xlabel("Epochs")
    plt.ylabel("Sum Rate")
