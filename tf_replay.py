...
def train_beamforming_transformer(config, pretrained_path: str = None, history_path: str = None):
    ...
    dataset_ini = ChannelDataset(num_samples=config.pbar_size * config.batch_size, num_users=config.num_users, num_tx=config.num_tx, P=config.SNR_power)
    dataloader_ini = DataLoader(dataset_ini, batch_size=config.batch_size, shuffle=True)
    replay_batch_buffer = []
    with th.no_grad():
        for batch_idx, H_tensor in enumerate(tqdm(dataloader_ini, desc="MMSE Evaluation")):
            H_tensor = H_tensor.to(device)
            H_mat = H_tensor[:, 0, :, :] + 1j * H_tensor[:, 1, :, :]
            W_mmse, _ = compute_mmse_beamformer(H_mat, config.num_users, config.num_tx, config.SNR_power, config.sigma2, device)
            mmse_rate = sum_rate(H_mat, W_mmse.transpose(-1,-2), sigma2=config.sigma2).item()
            replay_batch_buffer.append({
                'index': batch_idx,
                'data': H_tensor,  # Store the channel tensor
                'performance': mmse_rate  # Store the MMSE rate
            })

    # replay_batch_buffer = []  # Initialize the replay buffer
    batch_performance = []  # Store performance of each batch
    replay_batches_per_epoch = 0  # Number of replay batches to use per epoch
    ...
    if history_path is not None and pretrained_path is not None:
        ...
        ave_rate_update_history = saved_history.get('ave_rate_update_history', [])
    ...
    for epoch in range(start_epoch_prev, config.max_epoch):
        ...
        fine_tune_epoch = epoch - start_epoch_prev  # Fine-tune epoch index
        current_replay_proportion = min(0.5, 0.2 + (fine_tune_epoch // 10) * 0.01) 
        # Calculate current replay proportion (increases by 1% every 10 epochs)

        total_batches_needed = config.pbar_size
        replay_batches_per_epoch = int(total_batches_needed * current_replay_proportion)
        new_batches_per_epoch = total_batches_needed - replay_batches_per_epoch

        random_dataset = ChannelDataset(num_samples=new_batches_per_epoch * config.batch_size, num_users=config.num_users, num_tx=config.num_tx, P=config.SNR_power)
        random_dataloader = DataLoader(random_dataset, batch_size=config.batch_size, shuffle=True)

        ### construct replay buffer based on the batch-wise performance
        replay_batches = []
        if epoch > 0 and replay_batch_buffer:
            # Sort saved batches by their performance (sum-rate)
            sorted_replay = sorted(replay_batch_buffer, key=lambda x: x['performance'], reverse=True)
            # Take top batches according to current replay proportion
            top_batches = sorted_replay[:replay_batches_per_epoch]
            replay_batches.extend([batch['data'] for batch in top_batches])

        new_batch_count = 0
        batch_performance = []  # Reset performance tracking for this epoch
        ...
        epoch_rate_update = 0
        pbar_batches = 0
        pbar = tqdm(total=total_batches_needed, desc=f"Epoch {epoch+1}")
        max_rate = 0
        epoch_grad_norm = 0

        ### first replay batches
        for i, batch_data in enumerate(replay_batches):
            # Channel input: shape (B, 2, num_users, num_tx)
            H_tensor = batch_data.to(device)
            ...
            # rate_0 = sum_rate(H_mat, W_update, sigma2=config.sigma2)
            ...
            ave_rate_update = total_rate_update.item()
            ...
            batch_performance.append({
                'index': i,
                'data': H_tensor.detach().clone(),  # Store the tensor data
                'performance': ave_rate  # Store the performance metric
            })
        ...
        for batch in random_dataloader:
            ...
            batch_performance.append({
                'index': len(replay_batches) + new_batch_count,
                'data': H_tensor.detach().clone(),  # Store the tensor data
                'performance': ave_rate  # Store the performance metric
            })
        ...
        pbar.close()
        replay_batch_buffer = batch_performance.copy()
        ...
        print(f"Epoch {epoch+1} Average Sum Rate: {ave_pbar_rate:.4f}, Average Update Rate: {ave_pbar_rate_update:.4f}, Learning Rate: {current_lr:.2e}, teacher_weight: {teacher_weight:.2f}, Gradient Norm: {ave_grad_norm:.4f}")
        ave_rate_history.append(th.tensor(ave_pbar_rate))
        ave_rate_update_history.append(th.tensor(ave_pbar_rate_update))
        ...
        if (epoch + 1) % 10 == 0:
            ...
            saved_history = {
                ...
                'ave_rate_update_history': ave_rate_update_history,
    ...
    ave_rate_update_history = th.stack(ave_rate_update_history)
    ...
    th.save(ave_rate_update_history, f"rate_ave_update_history_{config.num_users}_{config.num_tx}_hybrid_attn_multi_layer.pth")
    ave_rate_update_history = th.load(f"rate_ave_update_history_{config.num_users}_{config.num_tx}_hybrid_attn_multi_layer.pth")
...
#     mmse_epoch = 200  # Epoch incorporating mmse supervision
    ...
    model_path_stage_1 = f"{config.num_users}_{config.num_tx}_hybrid_attn_multi_layer/best_model_6_6.pt"
    history_path_stage_1 = f"{config.num_users}_{config.num_tx}_hybrid_attn_multi_layer/saved_history_6_6.pt"
