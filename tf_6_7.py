...
def train_beamforming_transformer(config, pretrained_path: str = None, history_path: str = None):
    ...
    for epoch in range(start_epoch_prev, config.max_epoch):
        ...
        fine_tune_epoch = epoch - start_epoch_prev
        ## Soft switch learning policy.
        teacher_weight = max(0.0, 1 - (epoch / config.mmse_epoch))  # Linearly decrease from 1 to 0
        # teacher_weight = 0.0  

        # if fine_tune_epoch < config.wmmse_epoch:
        #     wmmse_weight = 1.0  # Use WMMSE samples only
        # else:
        #     wmmse_weight = 0.2

        wmmse_weight = max(0.2, 0.8 - (fine_tune_epoch / config.wmmse_epoch))  # Linearly decrease from 0.9 to 0.1
        ...
        dataset = ChannelDataset(num_samples=config.pbar_size * random_batch_size,
                             num_users=config.num_users,
                             num_tx=config.num_tx,
                             P=config.SNR_power)
        dataloader = DataLoader(dataset, batch_size=random_batch_size, shuffle=True)
        ...
        for batch in pbar:
            ...
            indices_wmmse = th.randperm(wmmse_set_size)[:wmmse_batch_size]
            ...
            ave_norm_wmmse = th.mean(th.norm(vec_w_wmmse, dim=1))
            print(f"Average norm of W_wmmse: {ave_norm_wmmse:.4f}")
            bp()
            ...
            H_combine = th.cat((H_random, H_wmmse), dim=0)  # (batch_size, num_users, num_tx)
            ...
            for t in range(1, config.T + 1):
                ...
                W_wmmse_out = W_next[random_batch_size:] # (wmmse_batch_size, 2 * num_users * num_tx)
                ...
                mse_loss = fun.mse_loss(W_wmmse_out, vec_w_wmmse)
                total_mse_loss += mse_loss           
            
            loss_unsupervised = - total_rate / config.T
            loss_supervised = total_mse_loss / config.T
            loss = (1 - wmmse_weight) * loss_unsupervised + (wmmse_weight * 20000) * loss_supervised ## supervised MSE
            print((1 - wmmse_weight) * loss_unsupervised.item(), (wmmse_weight * 20000) * loss_supervised.item())
            bp()
