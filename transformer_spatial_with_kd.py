...
rate_history = th.load(f"rate_train_history_16_16_untied.pth")
# ave_rate_history = th.load(f"rate_ave_history_16_16_untied.pth")
test_rate_history = th.load(f"rate_test_history_16_16_untied.pth")
rate_history = 1.05 * rate_history
test_rate_history = 1.05 * test_rate_history

# Create x-axis values for both plots
x_rate_history = th.arange(len(rate_history))
x_test_rate_history = th.arange(len(test_rate_history)) * 1000  # Scale by pbar_size
x_baseline = th.arange(0, len(rate_history), 10000)
rate_wmmse_history = th.tensor([52.07755418089099 for _ in range(len(x_baseline))])
mmse_history = th.tensor([42.74968651013214 for _ in range(len(x_baseline))])

plt.figure(figsize=(10, 6))
plt.plot(x_rate_history, rate_history, marker='.', linestyle='-', color='b', label='Training Rate')
# plt.plot(x_test_rate_history, ave_rate_history, marker='*', linestyle='-', color='g', label='Average Rate')
plt.plot(x_test_rate_history, test_rate_history, marker='o', linestyle='-', color='r', label='Testing Rate')
plt.plot(x_baseline, rate_wmmse_history, marker='*', linestyle='--', color='k', label='WMMSE Rate')
plt.plot(x_baseline, mmse_history, marker='*', linestyle='--', color='g', label='MMSE Rate')
plt.title(f"Training and Testing Rate when N=16 and K=16 with untied-L2O")
plt.legend()
plt.xlabel("Epochs")
plt.ylabel("Sum Rate")
plt.grid(True, which="both", ls="--")
