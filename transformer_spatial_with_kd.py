...
rate_history = th.load("D:/phd_2025_spring/Transformer_BF_with_prod-main/Transformer_BF_with_prod_home/hist_record/rate_train_history_16_16_self_attn.pth")
ave_rate_history = th.load("D:/phd_2025_spring/Transformer_BF_with_prod-main/Transformer_BF_with_prod_home/hist_record/rate_ave_history_16_16_self_attn.pth")
test_rate_history = th.load("D:/phd_2025_spring/Transformer_BF_with_prod-main/Transformer_BF_with_prod_home/hist_record/rate_test_history_16_16_self_attn.pth")
# Create x-axis values for both plots
# rate_history = rate_history + 1.5
# test_rate_history = test_rate_history + 1.5
x_train_rate_history = th.arange(len(rate_history))
x_test_rate_history = th.arange(len(ave_rate_history)) * 1000  # Scale by pbar_size
x_baseline_rate = th.arange(0, len(rate_history), 5000)
wmmse_history = th.tensor([52.07755418089099 for _ in range(len(x_baseline_rate))])
...
# plt.plot(x_test_rate_history, ave_rate_history, marker='*', linestyle='-', color='r', label='Average Rate')
...
plt.plot(x_test_rate_history, test_rate_history, marker='o', linestyle='-', color='r', label='Testing Rate')
plt.title(f"Training and Testing Rate when N=16 and K=16 with self attention")
plt.legend()
plt.xlabel("Epochs")
plt.ylabel("Sum Rate")
plt.grid(True, which="both", ls="--")
