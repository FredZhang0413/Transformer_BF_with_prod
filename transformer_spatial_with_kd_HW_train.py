...
def train_beamforming_transformer(config):
    ...
    for epoch in range(config.max_epoch):
        ...
        for batch in pbar:
            ...
            W_prev = W_mat_0.to(device)  # (B, num_tx, num_users)
            HW_prev = th.bmm(H_mat, W_prev)  # (B, num_users, num_users)
            total_rate = 0
            total_mse_loss = 0
            # max_rate = 0
            for _ in range(1, config.T + 1):
                # Our model takes the current channel H and the previous beamformer W_prev to predict the next beamformer.
                W_next = model(H_mat, HW_prev) # (B, beam_dim)
                ...
                W_prev = W_mat.to(device)  # (B, num_tx, num_users)
                HW_prev = th.bmm(H_mat, W_prev)  # (B, num_users, num_users)
