...
def sum_rate(H, W, sigma2=1.0):
    ...
    Args:
        ...
        W: Beamforming matrix (B, N, K) in complex form.
...
#############################################
# 2. Cross-Attention Block (unchanged).
#############################################
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, query, key, value):
        ...
        query: (B, L_q, d_model)
        key:   (B, L_k, d_model)
        value: (B, L_v, d_model) with L_k == L_v
        """
        B, L_q, _ = query.size()
        B, L_k, _ = key.size()
        q_ln = self.ln1(query)
        ...
        y = att @ V
        y = y.transpose(1,2).contiguous().view(B, L_q, -1)
        y = self.proj(y)
        y = self.resid_drop(y)
        out = query + y
...
#############################################
# 3. TransformerStep: uses ∂ sum_rate/∂W as Value tokens.
#############################################
class TransformerStep(nn.Module):
    def __init__(self, config, step_idx):
        ...
        """
        super().__init__()
        self.N = config.num_tx     # antennas = users
        self.K = config.num_users
        d = config.d_model

        # Antenna-level projections
        self.antenna_channel_proj = nn.Linear(self.K, d)
        self.antenna_beam_proj    = nn.Linear(self.K, d)
        self.antenna_prod_proj    = nn.Linear(self.K, d)
        self.pos_emb_ant = nn.Parameter(th.zeros(2 * self.N, d))
        self.cross_attn_ant = CrossAttentionBlock(d, config.n_head,
                                                  config.attn_pdrop, config.resid_pdrop)

        # User-level projections
        self.user_channel_proj = nn.Linear(self.N, d)
        self.user_beam_proj    = nn.Linear(self.N, d)
        self.user_prod_proj    = nn.Linear(self.K, d)
        self.pos_emb_user = nn.Parameter(th.zeros(2 * self.K, d))
        self.cross_attn_user = CrossAttentionBlock(d, config.n_head,
                                                   config.attn_pdrop, config.resid_pdrop)

        # Fusion MLP to output beamformer vector
        total_tokens = 2 * self.N + 2 * self.K
        self.out_proj = nn.Sequential(
            nn.Linear(total_tokens * d, 4 * d),
            nn.GELU(),
            nn.Dropout(config.resid_pdrop),
            nn.Linear(4 * d, 2 * d),
            nn.GELU(),
            nn.Dropout(config.resid_pdrop),
            nn.Linear(2 * d, config.beam_dim),
        )
    ...
    def forward(self, H, W_history, prod_history=None):
        """
        Args:
        ...
        # Retrieve last beamformer and make it require grad
        W_prev = W_history[-1]                  # (B, K, N), complex dtype
        W_prev = W_prev.clone().detach().requires_grad_(True)

        # Compute sum_rate and its derivative wrt W_prev
        rate_prev = sum_rate(H, W_prev, sigma2=config.sigma2)
        grad_W = th.autograd.grad(rate_prev.sum(), W_prev, create_graph=True)[0]
        # grad_W has shape (B, K, N), complex dtype

        # --- Antenna-level tokens ---
        # H tokens: real & imag, shape (B, 2N, K)
        H_ant = th.cat([H.real.transpose(1,2), H.imag.transpose(1,2)], dim=1)
        # Beamformer tokens
        W_ant = th.cat([W_prev.real.transpose(1,2), 
                        W_prev.imag.transpose(1,2)], dim=1)
        # Value tokens now use gradient instead of H*W: shape (B,2N,K)
        V_ant = th.cat([grad_W.real.transpose(1,2), 
                        grad_W.imag.transpose(1,2)], dim=1)

        # Project + position embed
        H_ant_proj = self.antenna_channel_proj(H_ant) + self.pos_emb_ant.unsqueeze(0)
        W_ant_proj = self.antenna_beam_proj(W_ant) + self.pos_emb_ant.unsqueeze(0)
        V_ant_proj = self.antenna_prod_proj(V_ant) + self.pos_emb_ant.unsqueeze(0)

        # Cross-attention
        x_a = self.cross_attn_ant(H_ant_proj, W_ant_proj, V_ant_proj)  # (B, 2N, d_model)

        # --- User-level tokens ---
        H_user = th.cat([H.real, H.imag], dim=1)         # (B, 2K, N)
        W_user = th.cat([W_prev.real, W_prev.imag], dim=1)
        V_user = th.cat([grad_W.real, grad_W.imag], dim=1)

        H_user_proj = self.user_channel_proj(H_user) + self.pos_emb_user.unsqueeze(0)
        W_user_proj = self.user_beam_proj(W_user) + self.pos_emb_user.unsqueeze(0)
        V_user_proj = self.user_prod_proj(V_user) + self.pos_emb_user.unsqueeze(0)

        x_u = self.cross_attn_user(H_user_proj, W_user_proj, V_user_proj)  # (B, 2K, d_model)
...
class BeamformingTransformerL2O(nn.Module):
    ...
    def forward(self, H, W0):
        ...
        for step in self.steps:
            vec = step(H, W_history, None)  # P_history unused
            # reconstruct complex W and append
            N, K = self.config.num_tx, self.config.num_users
