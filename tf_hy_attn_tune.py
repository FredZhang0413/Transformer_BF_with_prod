...
def train_beamforming_transformer(config, pretrained_path: str = None, history_path: str = None):
    ...
    for epoch in range(start_epoch_prev, config.max_epoch):
        ...
        if epoch < 200:
            ...
            progress = epoch - 200
            decay_span = 500
            student_weight = 0.5 + 0.75 * (1 - math.cos(math.pi * progress / decay_span))
        else:
            student_weight = 2.0
        ...
        for batch in pbar:
            ...
            th.nn.utils.clip_grad_norm_(model.parameters(), max_norm=max_norm_th)
...
if __name__ == "__main__":
    ...
    model_path_stage_1 = f"{config.num_users}_{config.num_tx}_hybrid_attn_multi_layer/best_model_APE.pt"
    history_path_stage_1 = f"{config.num_users}_{config.num_tx}_hybrid_attn_multi_layer/saved_history_APE.pt"
