...
class BeamformingTransformer(nn.Module):
    def __init__(self, config):
        ...
        # For antenna-level tokens: there are 8N tokens.
        self.pos_emb_ant = nn.Parameter(th.zeros(8 * self.N, config.d_model))
        # For user-level tokens: there are also 8N tokens.
        self.pos_emb_user = nn.Parameter(th.zeros(8 * self.K, config.d_model))
        ...
        # The fused feature dimension is 12N*d_model (8N from antenna + 8N from user).
        total_tokens = 8*self.N + 8*self.K
    ...
    def forward(self, H, W_prev):
        ...
        # C_ant has shape (B, N, 8N); we then transpose to obtain tokens of shape (B, 8N, N)
        C_ant = th.cat([H.real, H.imag, W_prev.real, W_prev.imag, Prod.real, Prod.imag, grad_W.real, grad_W.imag], dim=2)
        tokens_ant = C_ant.transpose(1, 2)  # (B, 8N, N)
        
        # Project tokens into d_model and add positional embeddings.
        tokens_ant_proj = self.ant_proj(tokens_ant)  # (B, 8N, d_model)
        tokens_ant_proj = tokens_ant_proj + self.pos_emb_ant.unsqueeze(0)
        
        # Perform self-attention on antenna-level tokens.
        x_a = self.self_attn_ant(tokens_ant_proj, tokens_ant_proj, tokens_ant_proj)  # (B, 8N, d_model)
        
        # ---------------------------
        # 2. User-level Self-Attention
        # Transpose H, W_prev, and Prod; then concatenate their real and imaginary parts along columns.
        # This forms C_user of shape (B, N, 8N), which is transposed to shape (B, 8N, N) so that each column is a token.
        H_T = H.transpose(1, 2)
        W_T = W_prev.transpose(1, 2)
        Prod_T = Prod.transpose(1, 2)
        grad_W_T = grad_W.transpose(1, 2)
        C_user = th.cat([H_T.real, H_T.imag, W_T.real, W_T.imag, Prod_T.real, Prod_T.imag, grad_W_T.real, grad_W_T.imag], dim=2)
        tokens_user = C_user.transpose(1, 2)  # (B, 8N, N)
        
        # Project tokens into d_model and add positional embeddings.
        tokens_user_proj = self.user_proj(tokens_user)  # (B, 8N, d_model)
        tokens_user_proj = tokens_user_proj + self.pos_emb_user.unsqueeze(0)
        
        # Perform self-attention on user-level tokens.
        x_u = self.self_attn_user(tokens_user_proj, tokens_user_proj, tokens_user_proj)  # (B, 8N, d_model)
        
        # ---------------------------
        # 3. Fusion and Output Prediction
        # Flatten both attention outputs and concatenate.
        x_a_flat = x_a.view(B, -1)   # (B, 8N * d_model)
        x_u_flat = x_u.view(B, -1)   # (B, 8N * d_model)
