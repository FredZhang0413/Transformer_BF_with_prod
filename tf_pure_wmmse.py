...
class WMMSEDataset(Dataset):
    def __init__(self, channels, beamformers):
        assert channels.shape[0] == beamformers.shape[0], "Number of channel and beamformer samples must match"
        self.channels = channels
        self.beamformers = beamformers
        self.num_samples = channels.shape[0]
        
    def __len__(self):
        return self.num_samples
    
    def __getitem__(self, idx):
        # Return a tuple: [channel, beamformer]
        return {
            'channel': self.channels[idx],  # Complex channel matrix
            'beamformer': self.beamformers[idx]  # Complex beamformer matrix
        }
...
def train_beamforming_transformer(config, pretrained_path: str = None, history_path: str = None):
    ...
    # save_dir = f"{config.num_users}_{config.num_tx}_6_7_store"
    # os.makedirs(save_dir, exist_ok=True)
    # best_model_path = os.path.join(save_dir, "best_model.pt")

    best_model_path = "best_model.pt"

    ### load the wmmse sample dataset in the fine-tune phase
    save_wmmse_dir = "wmmse_data"
    os.makedirs(save_wmmse_dir, exist_ok=True)
    pt_file_path = os.path.join(save_wmmse_dir, f"wmmse_{config.num_users}_{config.num_tx}.pt")
    # pt_file_path = f"wmmse_{config.num_users}_{config.num_tx}.pt"
    data = th.load(pt_file_path, weights_only=False)
    wmmse_channels = data['channels']
    wmmse_beamformers = data['beamformers']
    wmmse_beamformers = wmmse_beamformers.conj()
    wmmse_set_size = wmmse_channels.shape[0]
    target_channel_norm = th.sqrt(th.tensor(config.SNR_power * config.num_users * config.num_tx))
    current_norms = th.norm(wmmse_channels.view(wmmse_set_size, -1), dim=1)
    scaling_factors = target_channel_norm / current_norms
    wmmse_channels = wmmse_channels * scaling_factors.view(-1, 1, 1) ### normalized wmmse channel samples
    beamformer_norms = th.norm(wmmse_beamformers.view(wmmse_set_size, -1), dim=1)
    wmmse_beamformers = wmmse_beamformers / beamformer_norms.view(-1, 1, 1) ### normalized wmmse beamformer samples
    wmmse_channels = wmmse_channels.to(device)  # Move to device
    wmmse_beamformers = wmmse_beamformers.to(device)  # Move to device

    wmmse_dataset = WMMSEDataset(wmmse_channels, wmmse_beamformers)
    wmmse_loader = DataLoader(wmmse_dataset, batch_size=config.batch_size, shuffle=True)

    ### if written here, using the fixed wmmse proportions
    # wmmse_weight = 0.25
    # random_batch_size = int(math.ceil(config.batch_size * (1 - wmmse_weight)))  # random generated samples per batch (sum-rate objective in loss function)
    # wmmse_batch_size = config.batch_size - random_batch_size  # wmmse samples per batch (MSE objective in loss function)


    # ### ablation test
    # indices = th.randperm(wmmse_set_size)[:100]
    # batch_channels = wmmse_channels[indices] 
    # ave_norm_channel = th.mean(th.norm(batch_channels, dim=(1, 2)))
    # print(f"Average norm of channels in the batch: {ave_norm_channel:.4f}")
    # batch_beamformers = wmmse_beamformers[indices] 
    # ave_norm_bf = th.mean(th.norm(batch_beamformers, dim=(1, 2)))
    # print(f"Average norm of beamformers in the batch: {ave_norm_bf:.4f}")
    # # batch_beamformers = batch_beamformers.conj().transpose(-1, -2)  # (100, num_tx, num_users)  
    # avg_sum_rate = sum_rate(batch_channels, batch_beamformers.transpose(-1, -2), sigma2=1.0)
    # print(f"Average sum rate for the batch: {avg_sum_rate:.4f}")
    # bp()
    ...
    for epoch in range(start_epoch_prev, config.max_epoch):
        ...
        ### randomly generate new channel samples
        ...
        for batch in pbar:
            ...
            indices_wmmse = th.randperm(wmmse_set_size)[:wmmse_batch_size] ## randomly sample existing wmmse solutions
            H_wmmse = wmmse_channels[indices_wmmse] ## (wmmse_batch_size, num_users, num_tx)
            W_wmmse = wmmse_beamformers[indices_wmmse]  ## (wmmse_batch_size, num_users, num_tx)
            vec_w_wmmse = th.cat((th.real(W_wmmse).reshape(-1, config.num_users * config.num_tx), th.imag(W_wmmse).reshape(-1, config.num_users * config.num_tx)), dim=-1)
            vec_w_wmmse = vec_w_wmmse.reshape(-1, 2 * config.num_users * config.num_tx)  # (wmmse_batch_size, 2 * num_users * num_tx)

            # Channel input: shape (random_batch_size, 2, num_users, num_tx)
            H_tensor = batch.to(device)
            # Convert to complex channel matrix: (random_batch_size, num_users, num_tx)
            H_random = H_tensor[:, 0, :, :] + 1j * H_tensor[:, 1, :, :]
            ### concatenate H_wmmse and H_random
            H_combine = th.cat((H_wmmse, H_random), dim=0)  # (batch_size, num_users, num_tx)
            # print(f"Shape of H_combine: {H_combine.shape}")
            # bp()
            
            # Compute initial beamformer using MMSE.
            W0, _ = compute_mmse_beamformer(H_combine, config.num_users, config.num_tx, config.SNR_power, config.sigma2, device)
            ...
            normlized_W0_random = normlized_W0[wmmse_batch_size:] # (random_batch_size, 2 * num_users * num_tx)
            ...
            rate_0 = sum_rate(H_combine, W_mat_0, sigma2=config.sigma2)
            ...
            for t in range(1, config.T + 1):
                ...
                W_next = model(H_combine, W_prev)  # (B, beam_dim)
                W_wmmse_out = W_next[:wmmse_batch_size] # (wmmse_batch_size, 2 * num_users * num_tx)
                W_random_out = W_next[wmmse_batch_size:]  # (random_batch_size, 2 * num_users * num_tx)
                ...
                # W_random_mat = W_prev[:random_batch_size]  # (random_batch_size, num_users, num_tx) 
                rate = sum_rate(H_combine, W_mat, sigma2=config.sigma2)
                total_rate += rate

                ### supervised MSE loss from MMSE beamformer
                mse_loss_1 = fun.mse_loss(W_random_out, normlized_W0_random)
                total_mse_loss_1 += mse_loss_1

                ### supervised MSE loss from wmmse solutions
                mse_loss_2 = fun.mse_loss(W_wmmse_out, vec_w_wmmse)
                total_mse_loss_2 += mse_loss_2    
                        
            loss_unsupervised = - (1 - teacher_weight) * (total_rate / config.T)
            loss_supervised_mmse = (teacher_weight * 20000) * (total_mse_loss_1 / config.T)
            loss_supervised_wmmse = (wmmse_weight * 80000) * (total_mse_loss_2 / config.T)
            loss = loss_unsupervised + loss_supervised_mmse + loss_supervised_wmmse ## supervised MSE
            # print(loss_unsupervised.item(), loss_supervised_mmse.item(), loss_supervised_wmmse.item())
            # bp()
            ...
            pbar_batches += 1
            pbar.set_description(f"Epoch {epoch+1}, Avg Rate: {ave_rate:.4f}, Max Rate: {max_rate:.4f}, wmmse MSE: {loss_supervised_wmmse.item():.4f}")
...
class BeamformerTransformerConfig:
    def __init__(self, **kwargs):
        ...
        self.wmmse_epoch = kwargs['wmmse_epoch']
...
if __name__ == "__main__":
    ...
    d_model = 768 # Best one for 32*32
    n_head = 12 # Best one for 32*32
    n_layers = 5 # Best one for 32*32
    ...
    wmmse_epoch = 200  # Epoch incorporating wmmse supervision
...
#     mmse_epoch = 200  # Epoch incorporating mmse supervision
    ...
    config = BeamformerTransformerConfig(
        ...
        wmmse_epoch=wmmse_epoch
    )

    # model_path_stage_1 = f"{config.num_users}_{config.num_tx}_6_5_store/best_model_6_6.pt"
    # history_path_stage_1 = f"{config.num_users}_{config.num_tx}_6_5_store/saved_history_6_6.pt"
