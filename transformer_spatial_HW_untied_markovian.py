...
class TransformerStep(nn.Module):
    def __init__(self, config, step_idx):
        ...
        # Now only 2*N tokens, not t*2N
        self.pos_emb_ant = nn.Parameter(th.zeros(2 * self.N, d))
        self.cross_attn_ant = CrossAttentionBlock(d, config.n_head, config.attn_pdrop, config.resid_pdrop)
        ...
        # Now only 2*K tokens, not t*2K
        self.pos_emb_user = nn.Parameter(th.zeros(2 * self.K, d))
        self.cross_attn_user = CrossAttentionBlock(d, config.n_head, config.attn_pdrop, config.resid_pdrop)
    ...
    def forward(self, H, W_history, prod_history):
        ...
        # Cross‐attention at antenna‐level
        ...
        V_user = th.cat([P_prev.real, P_prev.imag], dim=1)  # (B, 2*K, K)
