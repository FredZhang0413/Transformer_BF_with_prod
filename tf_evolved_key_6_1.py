...
def train_beamforming_transformer(config, pretrained_path: str = None, history_path: str = None):
    ...
    save_dir = f"{config.num_users}_{config.num_tx}_5_30_store"
    os.makedirs(save_dir, exist_ok=True)
    best_model_path = os.path.join(save_dir, "best_model.pt")
    
    for epoch in range(config.max_epoch):

        current_epoch = start_epoch_prev + epoch ### if load the existing model
        #### if entering the late stage, routinely reset the optimizer
        if current_epoch >= config.late_epoch and (current_epoch - config.late_epoch) % config.moment_epoch == 0:
            # Create a new optimizer with the same parameters but reset momentum state
            print(f"Epoch {current_epoch}: Resetting Adam momentum state")
            current_lr = optimizer.param_groups[0]['lr']  # Preserve current learning rate
            new_lr = current_lr + config.late_incre_lr
            optimizer = configure_optimizer(model, current_lr, config.weight_decay)
            scheduler = th.optim.lr_scheduler.CosineAnnealingLR(
                optimizer,
                T_max=config.max_epoch - current_epoch, 
                eta_min=new_lr * 0.5 
            )
        ...
        if (epoch + 1) % 10 == 0:
            ...
            saved_history = {
                ...
                'start_epoch': start_epoch_prev + epoch + 1,
                'learning_rate': current_lr,
                'config': config.__dict__
            }

            th.save(saved_history, os.path.join(save_dir, "saved_history.pt"))
            print(f"History saved at epoch {start_epoch_prev + epoch + 1}")
...
class BeamformerTransformerConfig:
    def __init__(self, **kwargs):
        ...
        self.late_epoch = kwargs['late_epoch']
        self.late_incre_lr = kwargs['late_incre_lr']
        self.moment_epoch = kwargs['moment_epoch']
