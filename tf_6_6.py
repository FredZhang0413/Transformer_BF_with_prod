        ...
        for batch in pbar:
            ...
            if teacher_weight > 0.0:
                for t in range(1, config.T + 1):
                    ...
                    #### consider 1-step PGD refinement
                    loss_w = - rate
                    W_pre_update = W_mat.clone().detach().requires_grad_(True) 
                    grad_w = th.autograd.grad(loss_w, W_pre_update, retain_graph=True)[0]  # (B, num_tx, num_users) 
                    W_update = (W_pre_update + config.pgd_step_size * grad_w).detach().requires_grad_(True)  # (B, num_tx, num_users)
...
if __name__ == "__main__":
    ...
    pgd_step_size = 0.01  # Step size for PGD refinement
