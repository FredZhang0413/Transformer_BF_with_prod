...
class BeamformingTransformer(nn.Module):
    def __init__(self, config):
        ...
        # #### adopted if using the evolved key/value generation scheme
...
#############################################
# 6. Lookahead wrapper around any optimizer
#############################################
#### inner_loop steps: k; outer_loop_coefficient: alpha
class Lookahead(optim.Optimizer):
    def __init__(self, base_optimizer, alpha, k):
        if not 0.0 <= alpha <= 1.0:
            raise ValueError("alpha must be in [0,1]")
        if not 1 <= k:
            raise ValueError("k must be >= 1")
        self.optimizer = base_optimizer ### choose SGD_momentum to escape plateau
    ...
    def step(self, closure=None):
        """Perform inner optimizer step, then every k steps update slow weights."""
        total_norm = 0
        for group in self.param_groups:
        ...
        print(f"Fast SGD step {self.step_counter % self.k + 1}/{self.k}, "
            f"Gradient norm: {total_norm:.6f}")
    
        loss = self.optimizer.step(closure)
        self.step_counter += 1
        if self.step_counter % self.k == 0:
            # interpolate slow and fast weights
...
# def configure_optimizer(model, learning_rate, weight_decay):
    ...
    inner_lr = learning_rate * 1.0
    innner_optimizer = th.optim.SGD(
        optimizer_grouped_parameters, 
        lr=inner_lr, 
        momentum=momentum, 
        nesterov=nesterov
    ...
    optimizer = Lookahead(innner_optimizer, alpha=0.05, k=5)  # Lookahead wrapper around SGD
...
def train_beamforming_transformer(config, pretrained_path: str = None, history_path: str = None):
    ...
    for epoch in range(config.max_epoch):
        ...
        for batch in pbar:
            ...
            # print(loss_unsupervised.item(), 180000*loss_supervised.item())
            # bp()
